...
class AutoConnectionSettingsCommand(Command):
    ...
    def execute(self) -> None:
        ...
        result: DialogResult = dialog.show(
            self.__presenter.config.autoPlanetConnectionDistance,
            self.__presenter.showAutoConnections,
        )

        if result is DialogResult.Ok:
            autoConnectionDistance: int = dialog.getDistance()
            showAutoConnections: bool = dialog.getShowAutoConnections()
            self.__presenter.onAutoConnectionSettingChanged(
                autoConnectionDistance, showAutoConnections
            )
